...
export default function EditReceiptModal({ open, onClose, onSubmit, editingReceipt = null }) {
  ...
  // Populate form when editing receipt changes
  useEffect(() => {
  ...
  const handleSelectChange = (name, value) => {
    if (name === 'job_order') {
      ...
      setForm((f) => ({ 
        ...f, 
        [name]: value,
        receipt_amount: selectedJobOrder?.balance_amount || form.receipt_amount
      }));
    } else {
      setForm((f) => ({ ...f, [name]: value }));
    }
  ...
  useEffect(() => {
    ...
    if (!debouncedJobOrderNumber.trim() && !debouncedJobCustomerId.trim() && !debouncedJobPhone.trim()) {
      ...
      const filtered = jobOrders.filter(jobOrder => {
        ...
        return matchesJobOrder && matchesCustomerId && matchesPhone && (hasBalance || isCurrentJobOrder);
      });
      setFilteredJobOrders(filtered);
    }
  }, [debouncedJobOrderNumber, debouncedJobCustomerId, debouncedJobPhone, jobOrders, editingReceipt]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await receiptApi.updateReceipt(editingReceipt.id, formData);
      
      showNotification('Receipt updated successfully!', 'success');
      onSubmit(response, editingReceipt.id);
      
      onClose();
    } catch (error) {
      console.error('Error updating receipt:', error);
      const errorMessage = error.response?.data?.error || 'Failed to update receipt';
      ...
      <DialogContent className="!w-[600px] !max-w-[90vw] !h-auto !max-h-[85vh] p-0 overflow-hidden">
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 p-4">
          <DialogHeader className="pb-2 flex-shrink-0">
            <DialogTitle className="text-xl">Edit Receipt</DialogTitle>
            <DialogDescription className="text-sm mt-1">
              Update the receipt details below. Receipt ID: {editingReceipt.receipt_id}
            </DialogDescription>
          </DialogHeader>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="receipt_id" className="text-sm">Receipt ID</Label>
              <Input
                id="receipt_id"
                value={editingReceipt.receipt_id}
                disabled
                className="bg-gray-100 dark:bg-gray-700"
              />
              <p className="text-xs text-gray-500 dark:text-gray-400">Receipt ID cannot be changed</p>
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              {selectedJobOrder && form.receipt_amount && (
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  Remaining balance: ${(selectedJobOrder.balance_amount - parseFloat(form.receipt_amount || 0)).toFixed(2)}
                </p>
              )}
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <Select 
                value={editingReceipt.is_active ? "active" : "inactive"} 
                onValueChange={(value) => {
                  // This would need to be handled by the parent component
                  // or we could add a toggle status action here
                }}
              >
          ...
          <div className="bg-gray-50 dark:bg-gray-800 p-3 rounded-lg mt-2">
            <h4 className="font-medium text-gray-900 dark:text-gray-100 mb-2 text-sm">Receipt Information</h4>
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div>
              ...
              <div>
                ...
                <span className="ml-2 text-gray-900 dark:text-gray-100">
                  {editingReceipt.updated_at ? new Date(editingReceipt.updated_at).toLocaleString() : 'N/A'}
                </span>
              </div>
            </div>
          </div>
          ...
          <DialogFooter className="pt-2 border-t border-gray-200 dark:border-gray-700">
            ...
            <Button type="submit" disabled={loading || !form.job_order || !form.receipt_amount}>
